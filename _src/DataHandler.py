...
def oversample_dataset(input_vectors, labels, info=False, deep_info=False):
    ...
    if deep_info:
        print (all_shuf_labels)
        
    return shuf_vectors, shuf_labels
...
def create_dataset_buys(filename):
    ...
    if (sys.version_info > (3, 0)):
        data_type = np.dtype([('id_session', int), ('timestamp', 'S32'), ('id_item', int), ('price', int), ('quantity', int)])
    else:
        data_type = np.dtype([('id_session', int), ('timestamp', (str,32)), ('id_item', int), ('price', int), ('quantity', int)])
    data = np.loadtxt(filename, dtype=data_type, delimiter=',')
